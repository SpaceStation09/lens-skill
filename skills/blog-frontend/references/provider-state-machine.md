...
1. `connectWallet` 由 ConnectKit / Wagmi 等钱包库负责，本文件不定义其内部状态规则
...
| --------------------------- | ------------------------------ | -------------------------------------- | --------------------------------------------------- |
| `loginLens(accountAddress)` | 不允许；先连接钱包             | 允许；成功后进入 `authenticated`       | 允许；用于切换 Lens account（保持 `authenticated`） |
